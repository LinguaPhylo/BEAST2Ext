package outercore.evolution.sitemodel;
...
/**
 * Copied from BEAST2 core to allow the mutation parameter to be a Function instead of a RealParameter
 *
 * Site model with
 * o gamma site model,
...
public class SiteModel extends SiteModelInterface.Base {
     ...
     * @param beastInterface input
     */
    public void addCondition(final Input beastInterface) {
        if (beastInterface.get() == null) return;

        if (conditions == null) conditions = new ArrayList<>();

        conditions.add(((BEASTInterface)beastInterface.get()).getID());
    }
